...
create view group1 as select sum(numberofpeople),1 as agegroup14 from educationgrouping where ageofpeople>=14 and ageofpeople<20;
create view group2 as select sum(numberofpeople),1 as agegroup20 from educationgrouping where ageofpeople>=20 and ageofpeople<40;
create view group3 as select sum(numberofpeople),1 as agegroup40 from educationgrouping where ageofpeople>=40 and ageofpeople<60;
create view group4 as select sum(numberofpeople),1 as agegroup60 from educationgrouping where ageofpeople>=60 and ageofpeople<80;
select * from group1 a join group2 b on a.agegroup14=b.agegroup20 join group3 c on c.agegroup40=b.agegroup20  join group4 d where d.agegroup60=c.agegroup40;
drop view average1;
drop view average2;
drop view average3;
create view average1 as select round(avg(income),2) as testforaverage,1 as averagefor40  from  censusdata where age>=40 and age<60;
create view average2 as select round(avg(income),2)as testforaverage1,1 as averagefor20  from  censusdata where age>=20 and age<40;
--FOR INCOME WITHLOWEDUCATED ELIMINATING SENIOR CITIZENS
create view average3 as select round(avg(income),2)as testforaverage2,1 as averageforsenior  from  censusdata where education='Less than 1st grade';
select * from average1,average2,average3;
select round((testforaverage2-testforaverage)*100/testforaverage,2) from average1 a,average3 b where a.averagefor40=b.averageforsenior;
--/* 
