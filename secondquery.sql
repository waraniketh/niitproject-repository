...
create view incomeofmaxweek as select weeksworked,max(income) as testforweeksworked from censusdata group by weeksworked order by weeksworked desc limit 1;
--52 11465.2
...
select weeksworked,avg(income) from (select max(weeksworking) as workload from weeks) as a,censusdata b where a.workload=b.weeksworked group by weeksworked;
--1771.9899385777799
...
create view averageincomeoftop5 as select avg(income) as incomeoftop5,1 as keyforjoin from (select income from censusdata order by income desc limit 5) a;
--17272.04
create view averageincomeofpeoplewithmostworking as select weeksworked,avg(income) as averageincomeofmost,1 as key1 from (select max(weeksworking) as workload from weeks) as a,censusdata b where a.workload=b.weeksworked group by weeksworked;
--now finding diffenrence between top5 average and maximum income of most working people
select (incomeoftop5-averageincomeofmost)*100/averageincomeofmost from averageincomeofpeoplewithmostworking a,averageincomeoftop5 b a.key1=b.keyforjoin;
